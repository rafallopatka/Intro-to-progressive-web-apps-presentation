"use-strict";

var cacheKeys = [
  "/service-register.js",
  "/background-sync.js",
  "/tasks-repository.js",

  "/",
  "/manifest.json",
  "/assets/css/site.css",
  "/assets/js/site.js",

  "/inline.bundle.js",
  "/main.bundle.js",
  "/polyfills.bundle.js",
  "/styles.bundle.css",
  "/vendor.bundle.js",

  "/assets/images/categories/cleaning.png",
  "/assets/images/categories/family.png",
  "/assets/images/categories/home.png",
  "/assets/images/categories/medic.png",
  "/assets/images/categories/shopping.png",
  "/assets/images/icons/main.png",
  "/assets/images/icons/install-144x144.png",
  "/assets/images/icons/android-chrome-192x192.png",
  "/assets/images/icons/android-chrome-512x512.png",
  "/assets/images/icons/apple-touch-icon-120x120-precomposed.png",
  "/assets/images/icons/apple-touch-icon-120x120.png",
  "/assets/images/icons/apple-touch-icon-152x152-precomposed.png",
  "/assets/images/icons/apple-touch-icon-152x152.png",
  "/assets/images/icons/apple-touch-icon-180x180-precomposed.png",
  "/assets/images/icons/apple-touch-icon-180x180.png",
  "/assets/images/icons/apple-touch-icon-60x60-precomposed.png",
  "/assets/images/icons/apple-touch-icon-60x60.png",
  "/assets/images/icons/apple-touch-icon-76x76-precomposed.png",
  "/assets/images/icons/apple-touch-icon-76x76.png",
  "/assets/images/icons/apple-touch-icon-precomposed.png",
  "/assets/images/icons/apple-touch-icon.png",
  "/assets/images/icons/browserconfig.xml",
  "/assets/images/icons/favicon-16x16.png",
  "/assets/images/icons/favicon-194x194.png",
  "/assets/images/icons/favicon-32x32.png",
  "/assets/images/icons/favicon.ico",
  "/assets/images/icons/mstile-144x144.png",
  "/assets/images/icons/mstile-150x150.png",
  "/assets/images/icons/mstile-310x150.png",
  "/assets/images/icons/mstile-310x310.png",
  "/assets/images/icons/mstile-70x70.png",
  "/assets/images/icons/safari-pinned-tab.svg",
  "/assets/images/utils/loading.gif",

  "/node_modules/materialize-css/dist/css/materialize.min.css",
  "/node_modules/materialize-css/dist/css/materialize.min.css",
  "/node_modules/jquery/dist/jquery.min.js",
  "/node_modules/materialize-css/dist/js/materialize.min.js",
  "/node_modules/materialize-css/dist/fonts/roboto/Roboto-Thin.woff2",
  "/node_modules/materialize-css/dist/fonts/roboto/Roboto-Regular.woff2",
  "/node_modules/materialize-css/dist/fonts/roboto/Roboto-Light.woff2",
  "/node_modules/materialize-css/dist/fonts/roboto/Roboto-Medium.woff2",

  "https://fonts.googleapis.com/icon?family=Material+Icons",
  "https://fonts.googleapis.com/css?family=Roboto:400,300,100",
  "https://fonts.gstatic.com/s/roboto/v16/2tsd397wLxj96qwHyNIkxPesZW2xOQ-xsNqO47m55DA.woff2",
  "https://fonts.gstatic.com/s/materialicons/v21/2fcrYFNaTjcS6g4U3t-Y5ZjZjT5FdEJ140U2DJYC3mY.woff2",
  "https://fonts.gstatic.com/s/materialicons/v22/2fcrYFNaTjcS6g4U3t-Y5ZjZjT5FdEJ140U2DJYC3mY.woff2"
]

this.cacheKeys = cacheKeys;
